...
# @DEFAULT_UNSET
# @DESCRIPTION:
# Array with the syntax "repourl oid identifier branch" (still not sure where the oid comes from :/)
...
cargo-git_src_unpack() {
	...
	for line in "${CARGO_GIT[@]}"; do
		local giturl oid identifier branch
		read -r giturl oid identifier branch <<< "${line}"
